...
class ViewController: UIViewController, ARSCNViewDelegate{
    ...
    var minsymComLowLip: Float = .greatestFiniteMagnitude
    var maxsymComLowLip: Float = 0.0
    var minsymMouthVertical: Float = .greatestFiniteMagnitude
    var maxsymMouthVertical: Float = 0.0
    ...
    var minsymEyeHeight: Float = .greatestFiniteMagnitude
    var maxsymEyeHeight: Float = 0.0
    ...
    func setupUI() {
        ...
        // Initialize and configure evenLabel
        evenLabel = UILabel()
        evenLabel.translatesAutoresizingMaskIntoConstraints = false
        evenLabel.textColor = .white
        evenLabel.backgroundColor = UIColor.black.withAlphaComponent(0.5)
        evenLabel.textAlignment = .left
        evenLabel.numberOfLines = 0 // Allow multiple lines
        evenLabel.font = UIFont.systemFont(ofSize: 16, weight: .medium)
        ...
        let stackView = UIStackView(arrangedSubviews: [mouthLabel, eyeLabel, evenLabel])
    ...
    //
    ////            xData, at: 1095, stride: vertexStride, offset: vertexOffset),
    ...
    //
    //            let eye_indices = [1107, 1095, 1075, 1063]
    ...
    //                  let leftEyeTopVertex = getVertexPosition(from: vertexData, at: 1095, stride: vertexStride, offset: vertexOffset),
    //                  let leftEyeBottomVertex = getVertexPosition(from: vertexData, at: 1107, stride: vertexStride, offset: vertexOffset),
    //                  let rightEyeTopVertex = getVertexPosition(from: vertexData, at: 1075, stride: vertexStride, offset: vertexOffset),
    ...
    func updateFaceGeometry(faceAnchor: ARFaceAnchor, faceGeometry: ARSCNFaceGeometry, node: SCNNode) {
        ...
        if let vertexSource = faceGeometry.sources.first(where: { $0.semantic == .vertex }) {
            ...
            let mouthIndices = [190, 639,290,725] //  [172, 621] // [190, 639] // 290 and 725 for vertical to chin calc
            ...
            for i in chinIndices {
                self.placeDotOnVertex(at: i, with: vertexData, stride: vertexStride, offset: vertexOffset, on:
                                        node, color: .white)
            }
            ...
            //                }
            // 188, 637
            guard let leftMouthVertex = getVertexPosition(from: vertexData, at: 639, stride: vertexStride, offset: vertexOffset),  // 827, changed to 639
                  let rightMouthVertex = getVertexPosition(from: vertexData, at: 190, stride: vertexStride, offset: vertexOffset), // 397, changed to 190
                  ...
                  let leftEyeTopVertex = getVertexPosition(from: vertexData, at: 1094, stride: vertexStride, offset: vertexOffset), // 1095, changed to 1094
                  let leftEyeBottomVertex = getVertexPosition(from: vertexData, at: 1108, stride: vertexStride, offset: vertexOffset), // 1107
                  let rightEyeTopVertex = getVertexPosition(from: vertexData, at: 1076, stride: vertexStride, offset: vertexOffset), //1075, changed to 1076
            ...
            let CommissureWidth = (abs(leftMouthVertex.x) + abs(rightMouthVertex.x)) * 100  //CommissureWidth
            ...
            let LeftMouthVertical = (leftLowerLipVertex.y - leftChinVertex.y) * 100 //Left Lip to Chin Verticle
            ...
            let RightMouthVertical = (rightLowerLipVertex.y - rightChinVertex.y) * 100 //Right Lip to Chin Verticle
            ...
            let LeftCommPosLowLip = (sqrt(pow((leftMouthVertex.y - lowerLipMidlineVertex.y),2)+pow((leftMouthVertex.x-lowerLipMidlineVertex.x),2))) * 100 //Left Hypotenuse Commissure Position to Lower Lip Midline
            ...
            let RightCommPosLowLip = (sqrt(pow((rightMouthVertex.y - lowerLipMidlineVertex.y),2)+pow((rightMouthVertex.x-lowerLipMidlineVertex.x),2))) * 100 //Right Hypotenuse Commissure Position to Lower Lip Midline
            ...
            let insideRightEyeArea = calculatePolygonArea(vertices: insiderighteyeNodes) * 10000
            ...
            let insideLeftEyeArea = calculatePolygonArea(vertices: insidelefteyeNodes) * 10000
            ...
            //Symmetry Scores - AN Test
            
            //Symmetry Score for Commissure to Midline of Lower Lip
            let symComLowLip = (1-(abs(LeftCommPosLowLip-RightCommPosLowLip)/max(LeftCommPosLowLip,RightCommPosLowLip)))*100
            self.maxsymComLowLip = max(self.maxsymComLowLip, symComLowLip)
            self.minsymComLowLip = min(self.minsymComLowLip, symComLowLip)
            
            //Symmetry Score for Lip to Chin Vertical Distance
            let symMouthVertical = (1-(abs(LeftMouthVertical-RightMouthVertical)/max(LeftMouthVertical,RightMouthVertical)))*100
            self.maxsymMouthVertical = max(self.maxsymMouthVertical, symMouthVertical)
            self.minsymMouthVertical = min(self.minsymMouthVertical, symMouthVertical)
            
            //Symmetry Score for Eye Height
            let symEyeHeight = (1-(abs(leftEyeHeight-rightEyeHeight)/max(leftEyeHeight,rightEyeHeight)))*100
            self.maxsymEyeHeight = max(self.maxsymEyeHeight, symEyeHeight)
            self.minsymEyeHeight = min(self.minsymEyeHeight, symEyeHeight)
            ...
            DispatchQueue.main.async {
                self.updateMouthLabel(
                    ...
                    sym_com_low_lip: symComLowLip,
                    min_com_low_lip: self.minsymComLowLip,
                    max_com_low_lip: self.maxsymComLowLip,
                    sym_Mouth_Vertical: symMouthVertical,
                    min_sym_Mouth_Vertical: self.minsymMouthVertical,
                    max_sym_Mouth_Vertical: self.maxsymMouthVertical
                )
                
                self.updateEvenLabel(
                    sym_Mouth_Vertical: symMouthVertical,
                    min_sym_Mouth_Vertical: self.minsymMouthVertical,
                    max_sym_Mouth_Vertical: self.maxsymMouthVertical
                )
                ...
                self.updateEyeLabel(
                    ...
                    sym_eye_height: symEyeHeight,
                    min_sym_eye_height: self.minsymEyeHeight,
                    max_sym_eye_height: self.maxsymEyeHeight
                )
    ...
    @IBAction func resetMaxValues(_ sender: UIBarButtonItem) {
        ...
        minsymComLowLip = .greatestFiniteMagnitude
        maxsymComLowLip = 0.0
        minsymMouthVertical = .greatestFiniteMagnitude
        maxsymMouthVertical = 0.0
        minsymEyeHeight = .greatestFiniteMagnitude
        maxsymEyeHeight = 0.0
    ...
    func updateMouthLabel(
        ...
        sym_com_low_lip: Float, min_sym_com_low_lip: Float, max_sym_com_low_lip: Float,
        sym_mouth_vertical: Float, min_sym_mouth_vertical: Float,  max_sym_mouth_vertical: Float
    ) {
        let paragraphStyle = NSMutableParagraphStyle()
        ...
        let attributedString = NSMutableAttributedString(
            ...
            Mouth Metrics:
            \t\tCurrent\tMin\tMax
            Dental Show Area\t\t\(String(format: "%.2f", area)) cm²\t\(String(format: "%.2f", min_area)) cm²\t\(String(format: "%.2f", max_area)) cm²
            Commissure Width\t\t\(String(format: "%.2f", cwdistance)) cm\t\(String(format: "%.2f",min_cwdistance)) cm\t\(String(format: "%.2f", max_cwdistance)) cm
            L Low Lip Height\t\t\(String(format: "%.2f", left_mouth_vertical)) cm\t\(String(format: "%.2f",min_left_mouth_vertical)) cm\t\(String(format: "%.2f", max_left_mouth_vertical)) cm
            R Low Lip Height\t\t\(String(format: "%.2f", right_mouth_vertical)) cm\t\(String(format: "%.2f",min_right_mouth_vertical)) cm\t\(String(format: "%.2f", max_right_mouth_vertical)) cm
            L Comm Low Lip\t\t\(String(format: "%.2f", left_commissure_hypotenuse)) cm\t\(String(format: "%.2f",min_left_commissure_hypotenuse)) cm\t\(String(format: "%.2f", max_left_commissure_hypotenuse)) cm
            R Comm Low Lip\t\t\(String(format: "%.2f", right_commissure_hypotenuse)) cm\t\(String(format: "%.2f",min_right_commissure_hypotenuse)) cm\t\(String(format: "%.2f", max_right_commissure_hypotenuse)) cm
            Comm Symmetry\t\t\(String(format: "%.2f", sym_com_low_lip)) %\t\(String(format: "%.2f",min_sym_com_low_lip)) %\t\(String(format: "%.2f", max_sym_com_low_lip)) %
            Lip Height Symmetry\t\t\(String(format: "%.2f", sym_mouth_vertical)) %\t\(String(format: "%.2f",min_sym_mouth_vertical)) %\t\(String(format: "%.2f", max_sym_mouth_vertical)) %
            """,
            attributes: [.paragraphStyle: paragraphStyle, .foregroundColor: UIColor.white]
        )
    ...
    func updateEyeLabel(
        ...
        sym_eye_height: Float,
        min_sym_eye_height: Float,
        max_sym_eye_height: Float
    ) {
        let paragraphStyle = NSMutableParagraphStyle()
        ...
        let attributedString = NSMutableAttributedString(
            ...
            Left Eye Height           \t\(String(format: "%.2f", left_eye_height)) cm\t\(String(format: "%.2f", min_left_eye_height)) cm\t\(String(format: "%.2f", max_left_eye_height)) cm
            Right Eye Height          \t\(String(format: "%.2f", right_eye_height)) cm\t\(String(format: "%.2f", min_right_eye_height)) cm\t\(String(format: "%.2f", max_right_eye_height)) cm
            Left Eye Area\t\t\(String(format: "%.2f", left_eye_area)) cm²\t\(String(format: "%.2f", min_left_eye_area)) cm²\t\(String(format: "%.2f", max_left_eye_area)) cm²
            Right Eye Area\t\t\(String(format: "%.2f", right_eye_area)) cm²\t\(String(format: "%.2f", min_right_eye_area)) cm²\t\(String(format: "%.2f", max_right_eye_area)) cm²
            Lip Height Symmetry\t\t\(String(format: "%.2f", sym_eye_height)) %\t\(String(format: "%.2f",min_sym_eye_height)) %\t\(String(format: "%.2f", max_sym_eye_height)) %
            """,
            attributes: [.paragraphStyle: paragraphStyle, .foregroundColor: UIColor.white]
        )
